package com.test.api.client.okhttp;
...
import com.api.client.okhttp.factory.OkHttpAPIClientFactoryBuilder;
...
public class MockServiceOkHttpClientTest {
    ...
    @BeforeTest(groups = {"okhttp"})
    public void beforeTestSetup(final ITestContext context) {
        context.setAttribute("client", OkHttpAPIClientFactoryBuilder.getOkHttpAPIClientFactory().get());
        context.setAttribute("base_url", "http://localhost:11000");
    }

    @Test(groups = {"okhttp"}, invocationCount = 60)
    ...
    @Test(groups = {"okhttp"}, invocationCount = 10)
    ...
    public void signUpAndSearchWithNewClient(final ITestContext context) throws JsonProcessingException {
        this.client = OkHttpAPIClientFactoryBuilder.getOkHttpAPIClientFactory().get();
        this.baseUrl = (String) context.getAttribute("base_url");
