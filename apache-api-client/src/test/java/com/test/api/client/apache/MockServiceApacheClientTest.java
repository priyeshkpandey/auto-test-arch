package com.test.api.client.apache;

import com.api.client.apache.factory.ApacheAPIClientFactoryBuilder;
...
public class MockServiceApacheClientTest {
    private static final Random random = new Random();
    private static final ObjectMapper mapper = new ObjectMapper();
    private APIClient client;
    private String baseUrl;

    @BeforeTest(groups = {"apache"})
    public void beforeTestSetup(final ITestContext context) {
        context.setAttribute("client", ApacheAPIClientFactoryBuilder.getApacheAPIClientFactory().get());
        context.setAttribute("base_url", "http://localhost:11000");
    }

    @Test(groups = {"apache"}, invocationCount = 60)
